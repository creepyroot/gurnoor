  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (time - lastTime > tickRate) {
        ...
        // Self collision
        if (state.current.snake.some(segment => segment.x === head.x && segment.y === head.y)) {
          soundEngine.error();
          setGameState("gameover");
          return;
        }
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      for (let i = 0; i < CANVAS_SIZE; i += CELL) { ctx.moveTo(i, 0); ctx.lineTo(i, CANVAS_SIZE); }
      for (let i = 0; i < CANVAS_SIZE; i += CELL) { ctx.moveTo(0, i); ctx.lineTo(CANVAS_SIZE, i); }
      ctx.stroke();
      ...
      ctx.fillRect(state.current.food.x * CELL + 2, state.current.food.y * CELL + 2, CELL - 4, CELL - 4);
      
      // Draw Central inner dot for food
      ctx.fillStyle = "#FFF";
      ctx.shadowBlur = 0;
      ctx.fillRect(state.current.food.x * CELL + 8, state.current.food.y * CELL + 8, CELL - 16, CELL - 16);
      ...
      state.current.snake.forEach((segment, i) => {
        ...
        if (!isHead) {
           ...
           ctx.shadowBlur = 0;
           ctx.fillRect(segment.x * CELL + 6, segment.y * CELL + 6, CELL - 12, CELL - 12);
        }
      });
      ...
      <div className="w-full max-w-5xl px-4 flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 w-full">
          ...
          <div className="md:col-span-4 bg-zinc-950 border-2 border-neutral-800 rounded p-6 shadow-2xl flex flex-col justify-center">
            <h3 className="text-neutral-500 uppercase font-black tracking-widest text-[10px] mb-4">Operations Interface</h3>
            
            <div className="bg-black border border-neutral-800 p-4 rounded mb-6 text-center shadow-inner">
               <span className="block text-neutral-500 text-[10px] uppercase font-bold mb-1">Payload Extracted</span>
               <span className="text-4xl font-black text-green-500">{currentScore}</span>
            </div>

            <div className="space-y-3 mb-8">
              <div className="flex justify-between text-xs text-neutral-400 border-b border-neutral-800 pb-2">
                <span>SYSTEM STATUS:</span>
                <span className={gameState === 'gameover' ? 'text-brand-red' : gameState === 'playing' ? 'text-green-500' : 'text-brand-yellow'}>
                  {gameState === 'gameover' ? 'CRITICAL FAILURE' : gameState === 'playing' ? 'ACTIVE BREACH' : 'STANDBY'}
                </span>
              </div>
              <div className="flex justify-between text-xs text-neutral-400 border-b border-neutral-800 pb-2">
                <span>LENGTH:</span>
                <span className="text-white">{Math.floor(currentScore / 10) + 2} BLOCKS</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 max-w-[200px] mx-auto opacity-80 hover:opacity-100 transition-opacity">
               <div />
               <button onClick={() => changeDir(0, -1)} className="bg-neutral-800 hover:bg-green-500 hover:text-black p-4 rounded flex items-center justify-center transition-colors"><ChevronUp className="w-6 h-6" /></button>
               <div />
               <button onClick={() => changeDir(-1, 0)} className="bg-neutral-800 hover:bg-green-500 hover:text-black p-4 rounded flex items-center justify-center transition-colors"><ChevronLeft className="w-6 h-6" /></button>
               <button onClick={() => changeDir(0, 1)} className="bg-neutral-800 hover:bg-green-500 hover:text-black p-4 rounded flex items-center justify-center transition-colors"><ChevronDown className="w-6 h-6" /></button>
               <button onClick={() => changeDir(1, 0)} className="bg-neutral-800 hover:bg-green-500 hover:text-black p-4 rounded flex items-center justify-center transition-colors"><ChevronRight className="w-6 h-6" /></button>
            </div>
            <p className="text-center text-[10px] uppercase text-neutral-600 mt-4 leading-relaxed">Use Keyboard Arrows (WASD) or interface pads to direct the worm.</p>
          </div>
              ...
              <div className="absolute inset-0 z-20 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center text-center p-6">
                {gameState === "gameover" && (
                  <div className="mb-6 animate-pulse">
                    <h3 className="text-brand-red font-black text-3xl uppercase tracking-[0.2em] mb-2">CONNECTION TERMINATED</h3>
                    <p className="text-green-500 font-bold text-lg uppercase tracking-widest">Final Extraction: {currentScore}</p>
                  </div>
                )}
                ...
                >
                  ...
                  {gameState === "gameover" ? "Restart Protocol" : "Initialize Injection"}
                </button>
              </div>
            )}
