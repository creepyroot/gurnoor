...
export default function NeonPong() {
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (targetY > CANVAS_HEIGHT - PADDLE_HEIGHT)
      targetY = CANVAS_HEIGHT - PADDLE_HEIGHT;
    p1Y.current = targetY;
  };
  ...
  const update = () => {
    ...
    if (downPressed.current && p1Y.current < CANVAS_HEIGHT - PADDLE_HEIGHT)
    ...
    if (ball.current.dx > 0) {
      ...
      const timeToReach =
        (CANVAS_WIDTH - 20 - PADDLE_WIDTH - ball.current.x) / ball.current.dx;
      ...
      while (predictedY < 0 || predictedY > CANVAS_HEIGHT) {
        ...
        if (predictedY > CANVAS_HEIGHT)
          predictedY = CANVAS_HEIGHT * 2 - predictedY;
      }
      ...
      if (aiCenter < CANVAS_HEIGHT / 2 - 10) p2Y.current += PADDLE_SPEED * 0.4;
        ...
        p2Y.current -= PADDLE_SPEED * 0.4;
    }

    // clamp AI
    if (p2Y.current < 0) p2Y.current = 0;
    if (p2Y.current > CANVAS_HEIGHT - PADDLE_HEIGHT)
      p2Y.current = CANVAS_HEIGHT - PADDLE_HEIGHT;
    ...
    if (
      ...
      playSound("hit");
      const hitPoint =
        (ball.current.y - (p1Y.current + PADDLE_HEIGHT / 2)) /
        (PADDLE_HEIGHT / 2);
      ball.current.dy =
        hitPoint * BALL_SPEED_INIT * 1.5 + (Math.random() * 2 - 1);
    ...
    if (
      ...
      playSound("hit");
      const hitPoint =
        (ball.current.y - (p2Y.current + PADDLE_HEIGHT / 2)) /
        (PADDLE_HEIGHT / 2);
      ball.current.dy =
        hitPoint * Math.abs(ball.current.dx) * 0.8 + (Math.random() * 2 - 1);
  ...
  return (
    <section
      ref={containerRef}
      className="relative py-20 bg-black font-mono text-white flex flex-col items-center justify-center border-t-4 border-brand-red min-h-[600px] overflow-hidden"
    >
      ...
      <div className="w-full max-w-3xl px-4 flex flex-col items-center">
        ...
        <div className="flex w-full justify-between items-center mb-6">
          <div className="text-left">
            <div className="text-[10px] text-pink-500 uppercase tracking-widest">
              PLAYER 1
            </div>
        ...
        <div className="relative border-4 border-neutral-900 bg-neutral-950 p-2 shadow-[0_0_50px_rgba(255,0,85,0.15)] rounded touch-none">
          <canvas
            ...
            className="w-full h-auto bg-black max-w-full cursor-none border border-neutral-900 object-contain touch-none"
          ...
          {gameState === "idle" && (
            <div className="absolute inset-0 z-10 bg-black/60 flex flex-col items-center justify-center backdrop-blur-sm">
              ...
              <div className="mt-4 text-neutral-400 text-xs uppercase tracking-widest">
          ...
          {gameState === "gameover" && (
            <div className="absolute inset-0 z-10 bg-black/80 flex flex-col items-center justify-center backdrop-blur-sm">
              <div className="text-5xl font-black uppercase tracking-widest mb-2 drop-shadow-[4px_4px_0px_#FF0055]">
                {score.player >= 5 ? (
                  <span className="text-pink-500">YOU WIN!</span>
                ) : (
                  <span className="text-cyan-400">AI WINS!</span>
                )}
              </div>
              <div className="text-neutral-400 text-sm mb-8 uppercase tracking-widest">
                {score.player >= 5
                  ? "SECURITY NODE BYPASSED."
                  : "CONNECTION REJECTED."}
              </div>
              <button
                onClick={startGame}
