...
import { Briefcase, Calendar, Award, ExternalLink, ShieldCheck, Database, Bot } from "lucide-react";
...
export default function ExperienceTimeline() {
  ...
  return (
    <section id="experience" className="py-24 bg-black relative border-b-8 border-brand-red">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b-2 border-neutral-900 pb-8 mb-16">
          <div>
            <span className="font-mono text-xs text-brand-red tracking-[0.3em] uppercase block mb-2">[ CADRE HISTORY ]</span>
            <h2 className="text-5xl sm:text-7xl font-display font-black tracking-tighter text-white uppercase italic drop-shadow-[5px_5px_0px_#FFD700]">
              WORK <span className="text-brand-yellow drop-shadow-[5px_5px_0px_#FF0000]">EXPERIENCE</span>
            </h2>
          </div>
          <p className="mt-4 md:mt-0 font-mono text-[10px] text-neutral-500 uppercase max-w-sm md:text-right leading-relaxed">
            Deployment timeline tracking engineering sprints, high-volume automation audits, and secure local client integrations.
          </p>
        </div>
        ...
        <div className="space-y-16" style={{ perspective: "1500px" }}>
          {experiences.map((exp, index) => {
            const tel = telemetryData.find((t) => t.company === exp.company) || telemetryData[0];
            const TelIcon = tel.icon;

            return (
              <motion.div
                key={exp.company}
                initial={{ opacity: 0, scale: 0.84, rotateX: 18, rotateY: index % 2 === 0 ? -12 : 12, translateZ: -140, y: 80 }}
              ...
              >
                ...
                <div className="col-span-12 lg:col-span-7 flex flex-col justify-between pl-4">
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 mb-4">
                      <span className={`px-2.5 py-1 rounded text-[10px] font-mono tracking-widest uppercase font-black ${tel.pillBg}`}>
                        {exp.company}
                      </span>
                    ...
                    <ul className="space-y-3.5 mb-6">
                      {exp.bullets.map((bullet, idx) => (
                        <li key={idx} className="flex items-start text-neutral-300 font-sans text-sm leading-relaxed">
                  ...
                  {exp.company === "HackerOne" && (
                    <div className="mt-4">
                      ...
                      >
                        Launch CVE Exploit Matrix <ExternalLink className="w-4.5 h-4.5" />
                      </a>
                    </div>
                  )}
                </div>
                ...
                <div className="col-span-12 lg:col-span-5 bg-black border-2 border-neutral-900 rounded p-6 flex flex-col justify-between select-none">
                  <div>
                    <div className="flex justify-between items-center border-b border-neutral-900 pb-3 mb-4">
                      <span className="font-mono text-[9px] text-[#ef4444] font-black uppercase tracking-widest flex items-center gap-1.5">
                        <TelIcon className="w-4 h-4 text-brand-yellow" /> TELEMETRY ANALYSIS
                      </span>
                      <span className="font-mono text-[8px] text-neutral-505 font-bold">[RECORD_ID: #0{index + 1}]</span>
                    </div>

                    <div className="mb-4">
                      <span className="font-mono text-[10px] text-neutral-500 block">IMPACT ZONE</span>
                      ...
                      {tel.metrics.map((metric) => (
                        <div key={metric.label} className="space-y-1">
                          <div className="flex justify-between items-center text-[10px] font-mono">
                            ...
                            <span className="text-brand-yellow font-bold">{metric.value}</span>
                          </div>
                          ...
                          <div className="h-1.5 w-full bg-neutral-950 rounded-sm overflow-hidden flex gap-0.5">
                            ...
                            <div className="h-full flex-1 bg-brand-yellow rounded-sm" />
                            <div className={`h-full flex-1 rounded-sm ${index === 0 ? "bg-brand-yellow" : "bg-neutral-900"}`} />
                            <div className={`h-full flex-1 rounded-sm ${index === 1 ? "bg-brand-red" : "bg-neutral-900"}`} />
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="mt-6 border-t border-neutral-900 pt-4 flex items-center justify-between text-[10px] font-mono text-neutral-500 uppercase leading-none">
                    ...
                    <span className="text-brand-yellow font-black">STABLE STATE</span>
                  </div>
                </div>
