...
export default function CyberClicker() {
  ...
  const [upgrades, setUpgrades] = useState({ script: 0, botnet: 0, mainframe: 0 });
  ...
  const handleMainClick = (e: React.MouseEvent) => {
    ...
    floaty.className = "absolute pointer-events-none text-brand-yellow font-black font-mono animate-[floatUp_0.8s_ease-out_forwards]";
  ...
  return (
    <section ref={containerRef} className="relative py-20 bg-neutral-950 font-mono border-t-4 border-brand-yellow min-h-[600px] flex justify-center text-white">
      ...
      <div className="w-full max-w-4xl px-4 grid grid-cols-1 md:grid-cols-2 gap-8 z-10">
        ...
        <div className="text-center flex flex-col items-center justify-center p-8 border border-neutral-800 bg-black rounded relative overflow-hidden">
          ...
          <div className="text-neutral-400 font-bold tracking-widest uppercase mb-12">Total Bytes Extracted</div>
          ...
          >
            ...
            <span className="font-black">HACK <br/> PORTAL</span>
          </button>
        </div>

        {/* Right Side: Store */}
        <div className="flex flex-col gap-4 border border-neutral-800 bg-black p-6 rounded relative h-[500px] overflow-y-auto">
          <h3 className="text-xl font-black text-brand-red border-b border-neutral-800 pb-2 mb-4 uppercase sticky top-0 bg-black z-10">Black Market Store</h3>
...
function UpgradeBtn({ name, desc, cost, canAfford, onBuy }: any) {
  return (
    ...
    >
      ...
      <div className={`font-black ${canAfford ? 'text-brand-yellow' : 'text-neutral-500'}`}>
