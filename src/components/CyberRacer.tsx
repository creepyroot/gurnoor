...
export default function CyberRacer() {
  ...
  const setLaneByFraction = (fraction: number) => {
    const targetLane = Math.max(0, Math.min(LANES.length - 1, Math.floor(fraction * LANES.length)));
  ...
  useEffect(() => {
    ...
    const loop = (sysTime: number) => {
      ...
      const spawnRate = difficulty === "easy" ? 15000 : difficulty === "hard" ? 7000 : 10000;
      if (sysTime - s.lastObstacleTime > spawnRate / s.speed) {
        ...
        if (Math.random() > 0.4) {
           let l2;
           do { l2 = Math.floor(Math.random() * LANES.length); } while (l2 === lane);
           s.obstacles.push({ laneIndex: l2, z: MAX_DEPTH, type: 'red' });
           
           if (Math.random() > 0.6) {
             let l3;
             do { l3 = Math.floor(Math.random() * LANES.length); } while (l3 === lane || l3 === l2);
      ...
      const time = s.time;
      const getCurve = (z: number) => Math.sin(time * 0.002 + z * 0.002) * (z * 0.2);
      ...
      const sunGradient = ctx.createLinearGradient(0, horizon.py - 120, 0, horizon.py);
      ...
      ctx.fillStyle = "#050505";
      for (let i = 0; i < 5; i++) {
        ctx.fillRect(CANVAS_WIDTH/2 - 130, horizon.py - 10 - i*18, 260, 4 + i*2);
      }
      ctx.restore();
      ...
      // Polygon Helper
      const drawPoly = (points: any[], faceColor: string, edgeColor: string, isSolid: boolean = true) => {
      ...
      const drawBox = (cx: number, cy: number, cz: number, w: number, h: number, d: number, faceColor: string, edgeColor: string, glow: boolean = false) => {
        ...
        // Connecting Edges
        ctx.beginPath();
        ctx.moveTo(nbl.px, nbl.py); ctx.lineTo(fbl.px, fbl.py);
        ctx.moveTo(nbr.px, nbr.py); ctx.lineTo(fbr.px, fbr.py);
        ctx.moveTo(ntl.px, ntl.py); ctx.lineTo(ftl.px, ftl.py);
        ctx.moveTo(ntr.px, ntr.py); ctx.lineTo(ftr.px, ftr.py);
        ctx.stroke();
      ...
      const drawPagani = (cx: number, cy: number, cz: number) => {
        ...
        drawBox(cx, cy, cz + 50, 70, 4, 30, "rgba(5,5,5,0.9)", "#00FFFF", false);
        // Main Body Chassis (Low profile)
        drawBox(cx, cy + 4, cz, 76, 12, 110, "rgba(10,10,10,0.9)", "#00FFFF", false);
        // Wide Fenders (Left & Right)
        drawBox(cx - 36, cy, cz, 16, 20, 115, "rgba(5,5,5,0.9)", "#00FFFF", false);
        drawBox(cx + 36, cy, cz, 16, 20, 115, "rgba(5,5,5,0.9)", "#00FFFF", false);
        // Teardrop Glass Cabin (Tapered)
        drawBox(cx, cy + 16, cz + 10, 36, 14, 45, "rgba(0,180,255,0.2)", "#FFF", true);
        // Rear Deck Cover / Engine bay
        drawBox(cx, cy + 16, cz - 35, 30, 10, 40, "rgba(5,5,5,0.9)", "#00FFFF", false);
        // Signature Rear Spoiler (Huayra/Zonda style)
        drawBox(cx, cy + 34, cz - 55, 90, 3, 20, "rgba(15,15,15,0.9)", "#FF00FF", true);
        // Spoiler Struts
        drawBox(cx - 15, cy + 16, cz - 50, 4, 18, 12, "rgba(5,5,5,0.9)", "#FF00FF", false);
        drawBox(cx + 15, cy + 16, cz - 50, 4, 18, 12, "rgba(5,5,5,0.9)", "#FF00FF", false);
      ...
      frontObs.forEach(obs => {
        drawBox(LANES[obs.laneIndex], 0, obs.z, 65, 45, 60, "rgba(255,0,0,0.8)", "#FF3333", true);
      });
      ...
      <div className="w-full max-w-5xl px-4 flex flex-col items-center">
        <div className="text-center mb-8">
          ...
          <p className="text-[10px] md:text-sm text-neutral-400 max-w-xl mx-auto uppercase">
            Outrun the firewall. Avoid malignant packets blocking the transmission channels. Drag or tap across the screen to smooth steer.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 w-full">
          ...
          <div className="md:col-span-3 bg-zinc-950 border-2 border-neutral-800 rounded p-6 flex flex-col justify-center">
            <h3 className="text-neutral-500 uppercase font-black tracking-widest text-[10px] mb-4">Transmission Stats</h3>
            <div className="bg-black border border-neutral-800 p-4 rounded mb-6 text-center">
              <span className="block text-neutral-500 text-[10px] uppercase font-bold mb-1">Score</span>
              <span className="text-4xl font-black text-cyan-500">{currentScore}</span>
            </div>
            ...
            <div className="space-y-3 mb-8">
              ...
              <div className="flex justify-between text-xs text-neutral-400 border-b border-neutral-800 pb-2">
                <span>SPEED HZ:</span>
                <span className="text-white">{(state.current.speed).toFixed(1)}</span>
              </div>
              <div className="flex justify-between text-xs text-neutral-400 border-b border-neutral-800 pb-2">
                <span>STATUS:</span>
                <span className={gameState === 'gameover' ? 'text-brand-red' : gameState === 'playing' ? 'text-cyan-500' : 'text-neutral-500'}>
                  {gameState === 'gameover' ? 'CRASHED' : gameState === 'playing' ? 'ONLINE' : 'IDLE'}
                </span>
              </div>
            </div>

            <p className="text-center text-[10px] uppercase text-neutral-600 mt-4 leading-relaxed hidden sm:block">
              Use Keyboard Arrows (A/D) or Touch/Drag anywhere on the game display to steer smoothly across the 6-lane hyperway.
            </p>
          </div>
          ...
          >
              ...
              <div className="absolute inset-0 z-40 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center text-center p-6">
                {gameState === "gameover" && (
                  <div className="mb-6 animate-pulse">
                    <h3 className="text-brand-red font-black text-4xl uppercase tracking-[0.2em] mb-2">SYSTEM CRASH</h3>
                    <p className="text-cyan-500 font-bold text-xl uppercase tracking-widest">Final Score: {currentScore}</p>
                  </div>
                )}
                
                {(gameState === "start" || gameState === "gameover") && (
                  <div className="mb-8 flex gap-4">
                    <button onClick={() => setDifficulty("easy")} className={`px-4 py-2 border rounded text-xs font-bold uppercase tracking-widest transition-colors ${difficulty === 'easy' ? 'bg-cyan-500 text-black border-cyan-500' : 'bg-transparent text-neutral-400 border-neutral-700 hover:border-cyan-500 hover:text-cyan-500'}`}>Easy</button>
                    <button onClick={() => setDifficulty("normal")} className={`px-4 py-2 border rounded text-xs font-bold uppercase tracking-widest transition-colors ${difficulty === 'normal' ? 'bg-cyan-500 text-black border-cyan-500' : 'bg-transparent text-neutral-400 border-neutral-700 hover:border-cyan-500 hover:text-cyan-500'}`}>Normal</button>
                    <button onClick={() => setDifficulty("hard")} className={`px-4 py-2 border rounded text-xs font-bold uppercase tracking-widest transition-colors ${difficulty === 'hard' ? 'bg-brand-red text-black border-brand-red' : 'bg-transparent text-neutral-400 border-neutral-700 hover:border-brand-red hover:text-brand-red'}`}>Hard</button>
                  </div>
                )}
                ...
                >
                  {gameState === "gameover" ? <RotateCcw className="w-6 h-6" /> : <Play className="w-6 h-6" />}
            ...
            <canvas
              ...
              className="w-full h-full object-contain touch-none cursor-crosshair relative z-30"
