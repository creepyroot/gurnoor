...
export default function PayloadInjector() {
  ...
  const [status, setStatus] = useState<"idle" | "playing" | "success" | "hacked" | "failed">("idle");
      ...
      <div className="w-full max-w-4xl mx-auto px-4 relative z-10 flex flex-col items-center">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 text-brand-red text-xs font-black tracking-widest mb-2 border border-brand-red/30 px-3 py-1 rounded bg-brand-red/10">
            <Zap className="w-3.5 h-3.5 animate-pulse" /> NETWORK EXPLOIT MINIGAME
          </div>
          ...
          <p className="text-neutral-400 text-[11px] md:text-sm max-w-lg mx-auto uppercase">
            Timing matrix lock. Inject the phantom payload directly during the vulnerable server frame. Miss 3 times and the firewall burns you.
          </p>
        </div>
        ...
        <div className="w-full bg-black border-2 border-neutral-800 rounded p-6 md:p-10 shadow-2xl relative">
          ...
          <div className="flex justify-between items-center border-b border-neutral-900 pb-4 mb-8">
            <div className="font-bold text-xs uppercase flex items-center gap-2">
              <Cpu className="w-4 h-4 text-brand-yellow" /> FIREWALL LEVEL: <span className="text-brand-yellow">{level}/5</span>
            </div>
            <div className="flex items-center gap-2 text-xs font-bold">
               <ShieldAlert className="w-4 h-4 text-brand-red" />
               <div className="flex gap-1">
                 {[0, 1, 2].map(i => (
                    <div key={i} className={`w-3 h-3 rounded-full border border-brand-red ${i < strikes ? 'bg-brand-red' : 'bg-transparent'}`} />
                 ))}
               </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-24">
            {status === "idle" && (
              <button onClick={startGame} className="bg-brand-red hover:bg-red-500 text-black font-black text-sm uppercase px-8 py-4 rounded tracking-widest transition-all">
                INITIALIZE EXPLOIT
              </button>
            )}
            ...
            {status === "success" && (
               <div className="text-green-400 font-black text-xl animate-pulse tracking-widest uppercase flex flex-col items-center">
                 ...
                 <span className="text-[10px] text-neutral-500">PROCEEDING TO NEXT NODE...</span>
               </div>
            )}
            ...
            {status === "hacked" && (
               <div className="text-brand-yellow font-black text-2xl tracking-[0.3em] uppercase flex flex-col items-center">
                 ...
                 <span className="text-[10px] text-green-500 tracking-normal mt-2 border border-green-500/30 px-3 py-1 rounded">ROOT ACCESS GRANTED</span>
                 <button onClick={startGame} className="mt-4 text-[10px] text-neutral-400 hover:text-white underline tracking-normal">PLAY AGAIN</button>
               </div>
            )}
            ...
            {status === "failed" && (
               <div className="text-brand-red font-black text-2xl tracking-[0.2em] uppercase flex flex-col items-center">
                 ...
                 <span className="text-[10px] text-neutral-500 tracking-normal mt-2 border border-red-500/30 px-3 py-1 rounded">CONNECTION TERMINATED</span>
                 <button onClick={startGame} className="mt-4 text-[10px] text-neutral-400 hover:text-white underline tracking-normal">RETRY INFILTRATION</button>
               </div>
            )}
          </div>
        </div>
