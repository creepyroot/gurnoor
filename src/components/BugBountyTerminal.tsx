...
export default function BugBountyTerminal() {
  const hackerExp = portfolioData.experiences.find(
    (exp) => exp.company === "HackerOne",
  ) as Experience;
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b-2 border-neutral-900 pb-8 mb-12">
          ...
          <div className="mt-4 md:mt-0 font-mono text-xs text-neutral-400 border-2 border-brand-red bg-neutral-950 p-3 rounded shadow-[4px_4px_0px_#FFD700] max-w-sm flex items-center gap-3">
            ...
            <div>
              <span className="font-black text-[#FFD700] block uppercase">
                LEADERBOARD DISCLOSER
              </span>
              <span className="text-[10px] text-neutral-500">
                4+ Years active inside HackerOne platform auditing world-class
                systems
              </span>
            </div>
          </div>
          ...
          >
            ...
            <div className="col-span-12 flex justify-between items-center border-b border-neutral-900 pb-4 mb-2 px-2">
              <div className="flex items-center space-x-2">
                ...
                <span className="ml-3 font-mono text-[11px] text-neutral-500 tracking-wider">
                  HACKERONE_SIMULATOR_CORE_SHELL --creepy_root
                </span>
              </div>
            ...
            <div className="col-span-12 md:col-span-3 flex flex-col gap-2 p-2 bg-neutral-900/30 rounded border border-neutral-900 select-none">
              ...
              {vulnerabilities.map((vuln) => {
                const isActive =
                  selectedTarget.toLowerCase() === vuln.target.toLowerCase();
                return (
                  <button
                    key={vuln.target}
                  ...
                  >
                    <span className="flex items-center gap-2">
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${isActive ? "bg-brand-red" : "bg-neutral-700 animate-pulse"}`}
                      ></span>
                      {vuln.target}
                    </span>
                    {vuln.severity === "Critical" && (
                      <span
                        className={`px-1.5 py-0.5 text-[8px] rounded uppercase font-black tracking-widest ${
                          isActive
                            ? "bg-black text-brand-red"
                            : "bg-red-950/80 text-brand-red"
                        }`}
                      >
                        CRIT
                      </span>
                      ...
                      <span
                        className={`px-1.5 py-0.5 text-[8px] rounded uppercase font-black tracking-widest ${
                          isActive
                            ...
                            : "bg-yellow-950/80 text-brand-yellow"
                        }`}
                      >
                        HIGH
                      </span>
            ...
            <div className="col-span-12 md:col-span-12 lg:col-span-5 flex flex-col bg-black border border-neutral-900 rounded p-4 h-[350px] relative font-mono text-[10px] sm:text-xs">
              ...
              >
                ...
                {consoleLogs.map((log, index) => {
                  ...
                  if (log.includes("[ERROR]") || log.includes("[ALERT]"))
                    textClass = "text-brand-red font-black";
                  if (log.includes("[SUCCESS]"))
                    textClass = "text-brand-yellow font-black text-glow-yellow";
                  ...
                  return (
                    <div
                      key={index}
                      className={`leading-relaxed text-[11px] ${textClass}`}
                    >
                      {log}
                    </div>
                  );
            ...
            <div className="col-span-12 md:col-span-12 lg:col-span-4 flex flex-col justify-between p-4 bg-neutral-900/40 border-2 border-neutral-900 rounded select-none">
              <div>
                  ...
                  <div className="h-2 w-full bg-neutral-950 rounded overflow-hidden flex gap-1 p-0.5">
                    <div
                      className={`h-full flex-1 rounded-sm ${
                        ...
                        currentVulnerability.severity === "Medium"
                          ? "bg-brand-red"
                          : "bg-neutral-800"
                      }`}
                    />
                ...
                <div className="bg-black p-4 border border-neutral-850 rounded mb-4">
                  <span className="font-mono text-[10px] text-neutral-500 block mb-1 font-bold">
                    AUDIT BRIEF:
                  </span>
              ...
              <div className="bg-[#10b981]/5 p-3 border border-emerald-500/20 rounded flex items-center gap-3">
                ...
                <div>
                  <span className="font-mono text-[10px] text-neutral-400 block leading-none">
                    PATCH DISCLOSURE
                  </span>
            ...
            <div className="col-span-12 font-mono text-[10px] text-neutral-500 border-t border-neutral-900 pt-3 flex flex-col sm:flex-row justify-between uppercase mx-2 tracking-wide gap-2">
              <span>
                AUDIT DISCLOSURE POLICY: ALL EXPLOITS WERE RESOLVED SAFELY
                FOLLOWING RESPONSIBLE AND COORDINATED DISCLOSURE.
              </span>
              <span className="text-brand-red font-black">
                SECURED THROUGH ZERO TRUST BLUEPRINT
              </span>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 text-neutral-400 font-sans text-sm leading-relaxed border-l-4 border-brand-red pl-6 m-2">
          <div>
            <p>
              With over 4 years of dedicated freelance research, I’ve tracked
              and mapped security leakages alongside global engineering cohorts
              inside Google and Microsoft platforms. This provides a deep system
              strategy when planning server infrastructures.
            </p>
          </div>
          <div>
            <p>
              Thinking from an enemy’s vantage point allows me to design web
              applications with incredibly secure code sandboxing, encrypted
              payload delivery, resilient session tokens, and bulletproof
