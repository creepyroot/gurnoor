...
export default function DataBreaker() {
  ...
  const bricks = useRef<{x: number, y: number, w: number, h: number, status: number}[]>([]);
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (targetX + paddle.current.w > CANVAS_W) targetX = CANVAS_W - paddle.current.w;
  ...
  const draw = () => {
    ...
    ctx.fillRect(paddle.current.x, CANVAS_H - paddle.current.h - 10, paddle.current.w, paddle.current.h);
    ...
    bricks.current.forEach(b => {
      if (b.status === 1) {
         ctx.fillStyle = b.y < 60 ? "#FF0000" : (b.y < 100 ? "#FF00FF" : "#00FFFF");
  ...
  const update = () => {
    ...
    if ((keys["ArrowRight"] || keys["KeyD"]) && paddle.current.x + paddle.current.w < CANVAS_W) {
       paddle.current.x += paddle.current.speed;
    }
    ...
    // Wall bounce
    if (ball.current.x - ball.current.radius < 0 || ball.current.x + ball.current.radius > CANVAS_W) {
       ball.current.dx *= -1;
    }
    ...
    if (
      ...
      ball.current.x < paddle.current.x + paddle.current.w
       ...
       const hitPos = (ball.current.x - (paddle.current.x + paddle.current.w/2)) / (paddle.current.w/2);
  ...
  return (
    <section ref={containerRef} className="relative py-20 bg-neutral-950 font-mono text-white flex flex-col items-center border-t-4 border-[#00FFFF] overflow-hidden min-h-[600px]">
      ...
      <div className="w-full max-w-3xl px-4 flex flex-col items-center relative z-10">
        ...
        <div className="flex w-full justify-between items-center mb-4 max-w-[600px]">
           <div className="text-xl font-black text-white">SCORE: <span className="text-brand-yellow drop-shadow-[2px_2px_0px_red]">{score}</span></div>
        </div>
        ...
        <div className="relative border-4 border-neutral-800 bg-black p-2 shadow-[0_0_30px_rgba(0,255,255,0.1)] rounded w-full max-w-[600px] flex justify-center touch-none">
          ...
          {gameState === "gameover" && (
            <div className="absolute inset-0 bg-red-950/80 backdrop-blur-sm z-20 flex flex-col items-center justify-center">
              <div className="text-4xl text-brand-red font-black mb-4 drop-shadow-[5px_5px_0px_black] uppercase">CONNECTION LOST</div>
              <button 
                onClick={startGame}
          ...
          {gameState === "win" && (
            <div className="absolute inset-0 bg-emerald-950/80 backdrop-blur-sm z-20 flex flex-col items-center justify-center">
              <div className="text-4xl text-emerald-400 font-black mb-4 drop-shadow-[5px_5px_0px_black] uppercase animate-bounce">SERVER BREACHED</div>
              <button 
                onClick={startGame}
