...
export default function TerminalTyper() {
  ...
  const [words, setWords] = useState<
    { id: number; text: string; y: number; speed: number }[]
  >([]);
  ...
  const spawnWord = () => {
    ...
    const newWord = {
      ...
      text,
      y: -20,
      speed: 0.5 + Math.random() * 0.5 + score * 0.05,
    };
  ...
  return (
    <section
      ref={containerRef}
      className="relative py-20 bg-neutral-950 font-mono flex flex-col items-center border-t-4 border-cyan-500 min-h-[600px] overflow-hidden"
    >
      ...
      <div className="w-full max-w-2xl px-6 relative z-10 flex flex-col h-full items-center">
        ...
        <div className="flex w-full justify-between text-white font-black uppercase tracking-widest mb-4 z-20">
          ...
          <div className="flex gap-1">
            {Array.from({ length: 3 }).map((_, i) => (
              <Shield
                key={i}
                className={`w-5 h-5 ${i < hp ? "text-brand-red" : "text-neutral-700"}`}
              />
            ))}
          </div>
        </div>
        ...
        <div className="w-full h-[400px] bg-black border-2 border-neutral-800 relative overflow-hidden shadow-[inset_0_0_20px_rgba(0,255,255,0.05)] rounded z-20">
          {gameState === "idle" && (
            <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center backdrop-blur-sm z-30">
              <button
                onClick={startGame}
                className="bg-cyan-500 text-black px-6 py-3 font-black uppercase hover:bg-white transition-colors flex items-center gap-2"
              >
          ...
          {gameState === "gameover" && (
            <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center backdrop-blur-sm z-30">
              <div className="text-brand-red text-4xl font-black mb-2 animate-pulse">
                BREACH DETECTED
              </div>
              <div className="text-white mb-6">FINAL SCORE: {score}</div>
              <button
                onClick={startGame}
                className="bg-white text-black px-6 py-3 font-black uppercase hover:bg-cyan-500 transition-colors flex items-center gap-2"
              >
          ...
          {words.map((w) => (
            <div
              ...
              style={{
                top: `${w.y}px`,
                left: `${10 + Math.abs(Math.sin(w.id * 10)) * 80}%`,
              }}
            >
              {w.text}
            </div>
        ...
        <input
          ...
          placeholder={
            gameState === "playing" ? "TYPE TO DESTROY..." : "SYSTEM IDLE"
          }
